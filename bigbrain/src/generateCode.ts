import { assertNever } from "./helpers/assertNever.ts";
import { filterNullish } from "./helpers/filterNullish.ts";
import { unwrap } from "./helpers/unwrap.ts";
...
export function generateCode(nodes: AstNode[]): string {
  ...
  const gen = (node: AstNode): number => {
    switch (node.kind) {
      ...
      case "convert": {
        // @ts-ignore
        const arg: string = gen(node.arg) as string;
        // ASCII
        const charCode: number = arg.charCodeAt(0);
        operate(charCode, "");
